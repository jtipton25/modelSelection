...
## simulate the data
source('~/modelSelection/functions/simNottKohnSpatial.R')
...
n.o <- 50
m <- 100
## number of new observations to sample 
n <- 30
...
sigma.squared.epsilon <- 2.5
sigma.squared.eta <- 2
phi <- 5

simdata <- make.sim.data(n, m, p, cor.vec, beta, sigma.squared.epsilon, sigma.squared.eta, phi)
X.o <- simdata$X.o
Y.o <- as.vector(simdata$Y.o)
locs.o <- simdata$locs.o
idx <- order(locs.o)

X.full <- simdata$X.full
Y.full <- as.vector(simdata$Y.full)
locs.full <- simdata$locs.full

layout(matrix(1:2, nrow = 2))
plot(Y.o[idx] ~ locs.o[idx], type = 'l')
plot(Y.full, type = 'l')
