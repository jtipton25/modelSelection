...
##
## function to find MSPE for cv data using cross-validated selection of sigma^2_beta using bayesian regression 
##
...
make.grid.search.cv <- function(min.grid, max.grid, grid.size){
  ...
  for(i in 1:grid.size){
    ...
    MSPE.sim[i, 2] <- mean(sfSapply(1:k.fold, fun.mcmc.chain.cv, data.cv = data.cv, sigma.squared.beta = sigma.squared.beta[i]))
    cat(i, ' ')
  }
  return(MSPE.sim)
}

## change the parallelization to the highest level (outer loop)
...
sfStop()
plot(cross.validate.cv[, 2] ~ cross.validate.cv[, 1], type = 'l')
